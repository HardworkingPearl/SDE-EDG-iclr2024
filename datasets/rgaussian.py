...
class MultipleEnvironmentGauss(MultipleDomainDataset):
    def __init__(self, root, environments, dataset_transform, input_shape, num_classes):
        super(MultipleEnvironmentGauss, self).__init__()
...
@Datasets.register('rotatedgaussian')
class RotatedGaussian(MultipleEnvironmentGauss):
    def __init__(self, root, input_shape, num_classes):
        num_domains = 30
        environments = list(np.arange(num_domains))
        super(RotatedGaussian, self).__init__(root, environments, self.process_dataset, input_shape, num_classes)
